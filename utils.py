...
def compute_f0_rmvpe(wav_numpy, p_len=None, sampling_rate=44100, hop_length=512):
    import modules.RMVPEF0Predictor as RMVPEFf0
    
    f0_predictor = RMVPEFf0.RMVPEF0Predictor(hop_length=hop_length,f0_min=50,f0_max=1100, dtype=torch.float32, device=None,sampling_rate=sampling_rate,threshold=0.05)
    
    f0 = f0_predictor.compute_f0(wav=wav_numpy, p_len=p_len)
    return f0

def compute_f0_uv_rmvpe(wav_numpy, p_len=None, sampling_rate=44100, hop_length=512):
    import modules.RMVPEF0Predictor as RMVPEFf0
    
    f0_predictor = RMVPEFf0.RMVPEF0Predictor(hop_length=hop_length,f0_min=50,f0_max=1100, dtype=torch.float32, device=None,sampling_rate=sampling_rate,threshold=0.05)
    
    f0_uv = f0_predictor.compute_f0_uv(wav=wav_numpy, p_len=p_len)
    return f0_uv[0], f0_uv[1]
...
def get_speech_encoder(device, speech_encoder="whisper"):
    if speech_encoder == "whisper":
        from modules.WhisperPPG import WhisperPPG
