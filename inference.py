...
from modules.inferencer import Svc

logging.getLogger('numba').setLevel(logging.WARNING)
#chunks_dict = inferencer.read_temp("inference/chunks_temp.json")
...
def main():
    ...
    parser.add_argument('-se', '--speech_encoder', type=str, default="whisper",
                        help='Speech encoder.')
    ...
    svc_model = Svc(args.model_path, args.config_path, args.device, enhance, speech_encoder=speech_encoder)
    inferencer.mkdir(["raw", "results"])

    inferencer.fill_a_to_b(trans, clean_names)
    for clean_name, tran in zip(clean_names, trans):
        raw_audio_path = f"raw/{clean_name}"
        if "." not in raw_audio_path:
            raw_audio_path += ".wav"
        inferencer.format_wav(raw_audio_path)
        ...
        for spk in spk_list:
            ...
            for (slice_tag, data) in audio_data:
                ...
                if slice_tag:
                    ...
                    audio.extend(list(inferencer.pad_array(_audio, length)))
                    continue
                if per_size != 0:
                    datas = inferencer.split_list_by_n(data, per_size, lg_size)
                else:
                    datas = [data]
                for k, dat in enumerate(datas):
                    ...
                    _audio = inferencer.pad_array(_audio, per_length)
